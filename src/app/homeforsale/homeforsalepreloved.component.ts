...
@Component({
  ...
  templateUrl: './homeforsalepreloved.component.html',
  styleUrls:['./homeforsalepreloved.component.css']
})


export class homeForSalesPrecomponent implements OnInit { 
  homeForSales:any=[];
  filter:string='';
...
constructor(
    ...
    window.scrollTo(0, 0);
     ...
     this.apiService.getHomesList('/home_for_sales').subscribe((res:any)=>{
        ...
        for(var i=0;i<this.homeForSales.length;i++){
           if(this.homeForSales[i].record_type == "Pre-loved"){
              this.homeForSales[i].com_name = this.homeForSales[i].community.first_name;
              this.pre_homes_list.push(this.homeForSales[i]); 
           }
        }

        this.homeForSales = this.pre_homes_list;
        this.originalLength = this.homeForSales.length;

        window.localStorage.setItem('pre_home_sale_list',JSON.stringify(this.pre_homes_list));
         this.geoLocation("1");
  ...
  checkFilters(){
    if(this.states.length != 0){
      this.homeForSales=[];
     let homesData = JSON.parse(window.localStorage.getItem('pre_home_sale_list'));
